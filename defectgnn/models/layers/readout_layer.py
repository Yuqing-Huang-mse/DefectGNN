...
class ReadoutLayer(layers.Layer):
    def __init__(
        ...
        if use_batch_norm:
            ...
            for i in range(num_readout_fc_layers):
                self.fc_layers.extend([layers.Dense(hidden_size, activation=activation, use_bias=True, kernel_initializer=kernel_initializer)])
            self.fc_layers.extend([layers.Dense(num_targets, use_bias=False, kernel_initializer=kernel_initializer)])
